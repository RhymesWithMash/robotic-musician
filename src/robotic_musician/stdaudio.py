...
def _main():
    ...
    print('Creating and playing in small chunks...')
    ...
    print('Creating and playing in one large chunk...')
    sps = _SAMPLES_PER_SECOND
    notes = []
    inStream = instream.InStream('looney.txt')
    while not inStream.isEmpty():
...
def make_sound(samples):
    """
    Convert a list of float samples into a PyGame Sound object.
    """
    temp = [numpy.int16(s * float(0x7fff)) for s in samples]
    return pygame.sndarray.make_sound(numpy.array(temp, numpy.int16))
